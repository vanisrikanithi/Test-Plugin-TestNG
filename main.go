...
func main() {
	...
	if err := envconfig.Process("", &args); err != nil {
		logrus.Fatalf("Failed to process arguments: %s", err)
	}
	...
	if err := plugin.ValidateInputs(args); err != nil {
		logrus.Fatalf("Input validation failed: %s", err)
	}

	// Execute the plugin logic
	if err := plugin.Exec(context.Background(), args); err != nil {
		logrus.Fatalf("Plugin execution failed: %s", err)
	}

	logrus.Info("Plugin execution completed successfully")
}
